    ...
    open(component: Component, id?: number | any): NgbModalRef {
        ...
        if (id) {
            this.employeeService.find(id).subscribe((employee) => {
                if (employee.formDate) {
                    employee.formDate = {
                        year: employee.formDate.getFullYear(),
                        month: employee.formDate.getMonth() + 1,
                        day: employee.formDate.getDate()
                    };
                }
                if (employee.toDate) {
                    employee.toDate = {
                        year: employee.toDate.getFullYear(),
                        month: employee.toDate.getMonth() + 1,
                        day: employee.toDate.getDate()
                    };
                }
