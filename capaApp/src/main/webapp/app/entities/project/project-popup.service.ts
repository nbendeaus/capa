    ...
    open(component: Component, id?: number | any): NgbModalRef {
        ...
        if (id) {
            this.projectService.find(id).subscribe((project) => {
                if (project.start) {
                    project.start = {
                        year: project.start.getFullYear(),
                        month: project.start.getMonth() + 1,
                        day: project.start.getDate()
                    };
                }
                if (project.end) {
                    project.end = {
                        year: project.end.getFullYear(),
                        month: project.end.getMonth() + 1,
                        day: project.end.getDate()
                    };
                }
